...
function SelectContent({
  ...
  // Handle touch scroll properly on mobile
  const handleTouchMove = React.useCallback((e: React.TouchEvent) => {
    // Allow scroll to propagate within the content
    e.stopPropagation()
  }, [])
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        className={cn(
          "bg-popover text-popover-foreground data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 relative z-50 max-h-[min(40vh,280px)] min-w-[8rem] origin-(--radix-select-content-transform-origin) rounded-md border shadow-md",
          position === "popper" &&
      ...
      >
        <SelectPrimitive.Viewport
          className={cn(
            "p-1 max-h-[min(40vh,280px)] overflow-y-auto overscroll-contain",
            position === "popper" &&
...
// Scroll buttons removed - mobile-friendly native scroll is used instead
